...
def inverse_filtering(data, target_column, target_percentage, filter_cols, min_matches=50, min_combinations=1, max_combinations=3, max_results=100, max_single_results=5, excluded_columns=None):
    """
    ...
    def calculate_success_percentage(filtered_data):
    ...
    for n_filters in range(min_combinations, max_combinations + 1):
        ...
        for cols_combo in combinations(filter_cols, n_filters):
            ...
            for combo_type in ['>', '<']:
                # Crea una griglia di valori per ogni colonna
                grid_values = {}
                for col in cols_combo:
                    # Usa percentili per creare una griglia di valori
                    if pd.api.types.is_numeric_dtype(data[col].dtype):
                        unique_values = sorted(data[col].dropna().unique())
                        # Prendi un sottoinsieme di valori per efficienza
                        step = max(1, len(unique_values) // 10)
                        grid_values[col] = unique_values[::step]
                    else:
                        # Per colonne non numeriche, usa i valori unici
                        grid_values[col] = data[col].dropna().unique()
                
                # Genera combinazioni di valori per le colonne
                for i in range(min(100, 10**n_filters)):  # Limita il numero di tentativi
                    filter_conditions = []
                    ...
                    if len(filtered_data) >= min_matches:
                        success_percentage, count = calculate_success_percentage(filtered_data)
                        if success_percentage >= target_percentage:
                            # Per filtri singoli, controlla il limite per colonna
                            if n_filters == 1:
                                col_name = cols_combo[0]
                                if col_name not in single_results_count:
                                    single_results_count[col_name] = 0
                                
                                if single_results_count[col_name] < max_single_results:
                                    single_results_count[col_name] += 1
                                    results.append({
                                ...
                                # Per combinazioni multiple, aggiungi sempre
                                results.append({
                                    'filters': filter_conditions,
                                    'percentage': success_percentage,
                                    'count': count
