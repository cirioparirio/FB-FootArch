...
# Funzione per il filtraggio inverso completamente riprogettata
def efficient_inverse_filtering(data, target_column, target_percentage, filter_cols, min_matches=50, 
                               min_combinations=1, max_combinations=3, max_results=100, 
                               excluded_columns=None, progress_bar=None):
    """
    Versione efficiente della funzione di filtraggio inverso che analizza l'intero dataset.
    
    Utilizza un approccio a due fasi:
    1. Analisi preliminare di ogni colonna per identificare i valori più promettenti
    2. Combinazione dei filtri più promettenti per trovare le migliori combinazioni
    ...
    if not pd.api.types.is_numeric_dtype(data[target_column].dtype):
        ...
        try:
            ...
            if data[target_column].dtype == 'object':
                data[target_column] = data[target_column].apply(lambda x: 1 if str(x).upper() in ['V', 'TRUE', '1', 'YES', 'Y'] else 0)
    
    # Inizializza la barra di progresso
    if progress_bar is not None:
        progress_bar.progress(0, text="Fase 1: Analisi preliminare delle colonne...")
    
    # FASE 1: Analisi preliminare di ogni colonna per identificare i valori più promettenti
    promising_filters = []
    
    # Calcola la percentuale di successo globale
    total_success = data[target_column].sum()
    total_count = len(data)
    global_percentage = (total_success / total_count) * 100 if total_count > 0 else 0
    ...
    def calculate_percentage(filtered_data):
        ...
        return (success_count / count) * 100 if count > 0 else 0, count
    
    # Analizza ogni colonna numerica
    numeric_cols = [col for col in filter_cols if pd.api.types.is_numeric_dtype(data[col].dtype)]
    
    # Aggiorna la barra di progresso
    if progress_bar is not None:
        progress_bar.progress(0.1, text="Analisi delle colonne numeriche...")
    
    # Per ogni colonna numerica, trova i valori soglia che producono buoni risultati
    for i, col in enumerate(numeric_cols):
        # Calcola i percentili della colonna
        try:
            percentiles = [10, 20, 30, 40, 50, 60, 70, 80, 90]
            thresholds = [data[col].quantile(p/100) for p in percentiles]
            thresholds = [t for t in thresholds if not pd.isna(t)]
            
            # Testa ogni soglia
            for threshold in thresholds:
                # Test con operatore >
                filtered_data_gt = data[data[col] > threshold]
                percentage_gt, count_gt = calculate_percentage(filtered_data_gt)
                
                if percentage_gt >= target_percentage and count_gt >= min_matches:
                    promising_filters.append({
                        'filters': [{
                            'column': col,
                            'operator': '>',
                            'value': threshold
                        }],
                        'percentage': percentage_gt,
                        'count': count_gt
                    })
                
                # Test con operatore <
                filtered_data_lt = data[data[col] < threshold]
                percentage_lt, count_lt = calculate_percentage(filtered_data_lt)
                
                if percentage_lt >= target_percentage and count_lt >= min_matches:
                    promising_filters.append({
                        'filters': [{
                            'column': col,
                            'operator': '<',
                        ...
                        'percentage': percentage_lt,
                        'count': count_lt
                    })
            
            # Aggiorna la barra di progresso
            if progress_bar is not None:
                progress_percentage = 0.1 + 0.4 * (i + 1) / len(numeric_cols)
                progress_bar.progress(progress_percentage, text=f"Analisi colonna {col}... ({i+1}/{len(numeric_cols)})")
        
        except Exception as e:
            st.warning(f"Errore nell'analisi della colonna {col}: {e}")
            continue
    
    # Aggiorna la barra di progresso
    if progress_bar is not None:
        progress_bar.progress(0.5, text="Fase 2: Combinazione dei filtri più promettenti...")
    
    # FASE 2: Combinazione dei filtri più promettenti
    # Ordina i filtri promettenti per percentuale decrescente
    promising_filters.sort(key=lambda x: (-x['percentage'], -x['count']))
    
    # Limita il numero di filtri promettenti per migliorare le prestazioni
    top_promising_filters = promising_filters[:min(100, len(promising_filters))]
    
    # Aggiungi i filtri singoli ai risultati
    results.extend(top_promising_filters)
    
    # Se richiesto, genera combinazioni di filtri
    if max_combinations > 1:
        # Crea combinazioni di filtri
        for n_filters in range(2, min(max_combinations + 1, 4)):  # Limita a 3 filtri per prestazioni
            # Aggiorna la barra di progresso
            if progress_bar is not None:
                progress_percentage = 0.5 + 0.5 * (n_filters - 1) / (min(max_combinations, 3) - 1)
                progress_bar.progress(progress_percentage, text=f"Generazione combinazioni con {n_filters} filtri...")
            
            # Limita il numero di filtri da combinare
            filters_to_combine = top_promising_filters[:min(20, len(top_promising_filters))]
            
            # Genera tutte le possibili combinazioni di n_filters filtri
            from itertools import combinations
            for combo in combinations(range(len(filters_to_combine)), n_filters):
                # Estrai i filtri dalla combinazione
                combined_filters = []
                for idx in combo:
                    combined_filters.extend(filters_to_combine[idx]['filters'])
                
                # Verifica che non ci siano filtri duplicati sulla stessa colonna
                columns = [f['column'] for f in combined_filters]
                if len(columns) != len(set(columns)):
                    continue  # Salta combinazioni con colonne duplicate
                
                # Applica i filtri combinati
                filtered_data = data.copy()
                for filter_info in combined_filters:
                ...
                percentage, count = calculate_percentage(filtered_data)
                
                # Se soddisfa i criteri, aggiungi ai risultati
                if percentage >= target_percentage and count >= min_matches:
                    results.append({
            ...
            with tab2:
                ...
                if st.button("Esegui Filtraggio Inverso", type="primary", use_container_width=True):
                    ...
                    with st.spinner(f"Ricerca delle combinazioni di filtri per {target_column} > {target_percentage}% con almeno {min_matches} partite..."):
                        ...
                        start_time = time.time()
                        results = efficient_inverse_filtering(
                            df,
                            target_column,
                            target_percentage,
                            filter_columns,
                        ...
                        st.success(f"Analisi completata in {end_time - start_time:.2f} secondi!")
                    ...
                    5. **Clicca su "Esegui Filtraggio Inverso"** per avviare l'analisi
                    
                    L'applicazione cercherà combinazioni di filtri che producono una percentuale di successo superiore al target specificato.
                    
                    **Nota**: L'algoritmo è stato completamente riprogettato per funzionare in modo efficiente con l'intero dataset.
                    """)
        else:
